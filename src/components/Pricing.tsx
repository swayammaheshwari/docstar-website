  ...
  return (
    <div className="bg-white  pt-24 pb-16 px-4 sm:px-6 lg:px-16">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-extrabold text-gray-800 mb-2">Choose the plan that’s right for you</h2>
        <p className="text-gray-500 text-md">Simple, transparent pricing for individuals, teams, and enterprises.</p>
      </div>
      ...
      <div className="grid  grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan, index) => (
          ...
          >
              ...
              {plan.features.map((feature, i) => {
                ...
                return (
                  <li key={i} className="flex items-start">
                    <span className={`${hasCheck ? 'text-green-600' : 'text-red-500'} font-bold mr-2`}>
                      {hasCheck ? '✓' : hasCross ? '✗' : ''}
                    </span>
                    {cleanedText}
                  </li>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm text-gray-700">
                  {plan.integrations.map((icon, i) => (
                    <span key={i} className="bg-gray-100 px-2 py-1 rounded-md border border-gray-200">{icon}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <button
              className="mt-6 w-full py-2 rounded-xl text-white font-semibold bg-gray-800 hover:opacity-90 transition"
            >
